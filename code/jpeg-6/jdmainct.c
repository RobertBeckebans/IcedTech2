 ...
 */
{
   ...
   */
	jmain->xbuffer[ 0 ] = ( JSAMPIMAGE )( *cinfo->mem->alloc_small )( ( j_common_ptr )cinfo, JPOOL_IMAGE, cinfo->num_components * 2 * SIZEOF( JSAMPARRAY ) );
		...
		JDIMENSION                             out_rows_avail )
{
   ...
   */
	...
	( *cinfo->post->post_process_data )( cinfo, jmain->buffer, &jmain->rowgroup_ctr, rowgroups_avail, output_buf, out_row_ctr, out_rows_avail );
   ...
   */
	switch( jmain->context_state )
	{
		case CTX_POSTPONED_ROW:
			...
			( *cinfo->post->post_process_data )( cinfo, jmain->xbuffer[ jmain->whichptr ], &jmain->rowgroup_ctr, jmain->rowgroups_avail, output_buf, out_row_ctr, out_rows_avail );
	...
	jinit_d_main_controller( j_decompress_ptr cinfo, boolean need_full_buffer )
{
   ...
   */
		 ...
		 ci++, compptr++ )
	{
		rgroup = ( compptr->v_samp_factor * compptr->DCT_scaled_size ) /
			cinfo->min_DCT_scaled_size; /* height of a row group of component */
		jmain->buffer[ ci ] = ( *cinfo->mem->alloc_sarray )( ( j_common_ptr )cinfo, JPOOL_IMAGE, compptr->width_in_blocks * compptr->DCT_scaled_size, ( JDIMENSION )( rgroup * ngroups ) );
	}
}
