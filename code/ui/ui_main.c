		...
		while( s && *s && count < len )
		{
			...
			else
			{
				...
				if( style == ITEM_TEXTSTYLE_SHADOWED || style == ITEM_TEXTSTYLE_SHADOWEDMORE )
				{
					...
					Text_PaintChar( x + ofs, y - yadj + ofs, glyph->imageWidth, glyph->imageHeight, font, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph );
					trap_R_SetColor( newColor );
					colorBlack[ 3 ] = 1.0;
				}
				Text_PaintChar( x, y - yadj, glyph->imageWidth, glyph->imageHeight, font, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph );
	...
	if( text )
	{
		...
		while( s && *s && count < len )
		{
			...
			else
			{
				...
				{
					Text_PaintChar( x, y - yadj, glyph2->imageWidth, glyph2->imageHeight, font, useScale, glyph2->s, glyph2->t, glyph2->s2, glyph2->t2, glyph2->glyph );
				}
...
static void UI_DrawLoadStatus( rectDef_t* rect, vec4_t color, int align )
{
	...
	if( expectedHunk > 0 )
	{
		...
		if( percentDone > 0.97 )
		{ // never actually show 100%, since we are not in the game yet
			percentDone = 0.97;
		}
...
void UI_ParseSavegame( int index )
{
	...
	if( ver < 9 )
	{ // don't try anything for really old savegames
		trap_FS_FCloseFile( f );
...
static void UI_LoadSavegames( char* dir )
{
	...
	if( uiInfo.savegameCount )
	{
		...
		for( i = 0; i < uiInfo.savegameCount; i++ )
		{
			...
			{ // ignore some savegames that have special uses and shouldn't be loaded by the user directly
				i--;
