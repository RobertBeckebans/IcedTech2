...
void Script_NotebookShowpage( itemDef_t* item, char** args )
{
	...
	if( Int_Parse( args, &inc ) )
	{
		...
		{ // opening
			if( pages && !curpage )
			{            // only open to cover if no pages exist
		...
		if( inc )
		{
			...
			if( inc > 0 )
			{
				...
				if( i < NOTEBOOK_MAX_PAGES )
				{ // a valid page was found
					curpage = newpage;
				}
			}
			else
...
void Script_Map( itemDef_t* item, char** args )
{
	const char* val;
	if( String_Parse( args, &val ) )
	{
		DC->executeText( EXEC_APPEND, va( "map %s ; ", val ) );
	}
}

void Script_LoadAutoSave( itemDef_t* item, char** args )
{
	const char* val;
	if( String_Parse( args, &val ) )
	{
		DC->executeText( EXEC_APPEND, "loadgame autosave;" );
	}
}

void Script_Disconnect( itemDef_t* item, char** args )
{
	const char* val;
	if( String_Parse( args, &val ) )
	{
		DC->executeText( EXEC_APPEND, "disconnect;" );
	}
}

void Script_MainEscapeHandler( itemDef_t* item, char** args )
{
	int paused = trap_Cvar_VariableValue( "cl_paused" );
...
commandDef_t commandList[] = {
	...
	{ "exec", &Script_Exec },                     // group/name
												  // jmarshall
	{ "map", &Script_Map },
	{ "loadautosave", &Script_LoadAutoSave },
	{ "disconnect", &Script_Disconnect },
	{ "mainEscapeHandler", &Script_MainEscapeHandler },
	// jmarshall end
