...
static worldMaterial_t s_worldMaterials[ 1024 ];
static int             s_numWorldMaterials = 0;

int      c_subdivisions;
int      c_gridVerts;
model_t* currentBrushModel = NULL;

// Full list of shaders referenced by the world.
static int       s_numWorldShaders = 0;
static shader_t* s_worldShaders[ MAX_SHADERS ];

//===============================================================================

static void AddShaderToWorldShaders( shader_t* shader )
{
	if( shader->index >= MAX_SHADERS )
	{
		Com_Error( ERR_FATAL, "Failed to register world shader(too many shaders)\n" );
		return;
	}

	for( int i = 0; i < s_numWorldShaders; i++ )
	{
		if( s_worldShaders[ i ] == shader )
			return;
	}

	s_worldShaders[ s_numWorldShaders++ ] = shader;
}

int RE_GetNumWorldShaders( void )
{
	return s_numWorldShaders;
}

qhandle_t RE_GetWorldShader( int index )
{
	return s_worldShaders[ index ]->index;
}
...
static shader_t* ShaderForShaderNum( int shaderNum, int lightmapNum )
{
	...
	AddShaderToWorldShaders( shader );
...
static void R_LoadSubmodels( lump_t* l )
{
	...
	for( i = 0; i < count; i++, in++, out++ )
	{
		...
		//model->dxrMesh[1] = GL_LoadDXRMesh(out->firstSurface, out->numSurfaces, i + 2, qtrue);
	}
}
...
/*
=================
R_LoadWorldMaterialOverrides
=================
*/
void R_LoadWorldMaterialOverrides( const char* name )
{
	char  filename[ 512 ];
	char *buffer, *parse_buffer;

	s_numWorldMaterials = 0;

	COM_StripExtension( name, filename );
	COM_DefaultExtension( filename, sizeof( filename ), ".worldmtr" );

	// load it
	ri.FS_ReadFile( filename, ( void** )&buffer );
	if( !buffer )
	{
		ri.Printf( PRINT_WARNING, "Map %s doesn't have a world material\n", name );
		return;
	}
	...
	while( 1 )
	{
		char* token = COM_Parse( &parse_buffer );
		if( !token[ 0 ] )
			break;

		worldMaterial_t* mtr = &s_worldMaterials[ s_numWorldMaterials++ ];

		strcpy( mtr->name, token );

		token = COM_Parse( &parse_buffer );

		if( !strcmp( token, "reflect" ) )
		{
			mtr->reflect = qtrue;
		}
		else
		{
			ri.Error( ERR_FATAL, "Unknown token in world materials %s\n", token );
		}
	}
...
{
	for( int i = 0; i < s_numWorldMaterials; i++ )
	{
		if( !strcmp( s_worldMaterials[ i ].name, name ) )
		{
			return &s_worldMaterials[ i ];
		}
	}
	return NULL;
}
...
void RE_LoadWorldMap( const char* name )
{
	...
	R_LoadWorldMaterialOverrides( name );
