...
void trap_SetPersistant( PlayerPersistant_t* persistant )
{
	syscall( G_SETPERSISTANT, persistant );
}

void trap_GetPersistant( PlayerPersistant_t* persistant )
{
	syscall( G_GETPERSISTANT, persistant );
}

qhandle_t trap_OpenSaveForWrite( const char* name )
{
	return syscall( G_OPENSAVEWRITE, name );
}

qhandle_t trap_OpenSaveForRead( const char* name )
{
	return syscall( G_OPENSAVEREAD, name );
}

qboolean trap_SV_GetSaveGameName( const char* name )
{
	return syscall( G_GETSAVEGAMENANE, name );
}